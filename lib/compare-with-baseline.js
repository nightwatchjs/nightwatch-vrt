...
module.exports = async function compareWithBaseline(
  ...
  try {
    ...
    } else if (!identical && (always_save_diff_screenshot === true || process.argv.includes('--update-snapshots'))) {
  ...
  if (result !== true) {
    reporter.insertData({browserName, testModule, assertionName, completeBaselinePath, completeDiffPath, completeLatestPath});
  }
