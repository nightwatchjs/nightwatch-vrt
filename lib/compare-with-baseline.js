...
module.exports = async function compareWithBaseline(
  ...
  const {
    ...
    prompt,
    updateScreenshots
  } = getVrtSettings(context.client.settings, overrideSettings);
  ...
  const clientReporter = context.client.reporter;
  ...
  if (result !== true) {
    ...
    const relativeLatestPath = `/${path.relative(
      process.cwd(),
      completeLatestPath
    )}`;
    reporter.insertData(clientReporter, {
      browserName,
      testModule,
      assertionName,
      completeBaselinePath: relativeBaselinePath,
      completeDiffPath: relativeDiffPath,
      completeLatestPath: relativeLatestPath,
      diff: diff.percent
    });
  }
